...
var clientDb = require('./clientsinmem');
...
server.deserializeClient(function(id, done) {
    clientDb.clients.find(id, function(err, client) {
...
server.grant(oauth2orize.grant.code(
    ...
    function(client, redirectURI, user, ares, done) {
        ...
        clientDb.authorizationCodes.save(code, client.id, redirectURI, user.id, function(err) {
            if (err) { return done(err); }
            done(null, code);
        });
    }
...
server.exchange(oauth2orize.exchange.code(function(client, code, redirectURI, done) {
    clientDb.authorizationCodes.find(code, function(err, authCode) {
        if (err) { return done(err); }
        if (client.id !== authCode.clientID) { return done(null, false); }
        if (redirectURI !== authCode.redirectURI) { return done(null, false); }

        var token = utils.uid(256)
        clientDb.accessTokens.save(token, authCode.userID, authCode.clientID, function(err) {
            if (err) { return done(err); }
            done(null, token);
        });
    });
...
exports.authorization = [
    ...
    // Second one calls the authorization method on the OAuth server to check the client
    server.authorization(function(clientID, redirectURI, done) {
        clientDb.clients.findByClientId(clientID, function(err, client) {
            if (err) { return done(err); }
...
// decide on whether a user is authorized
//
...
exports.decision = [
    // First callback/middleware ensures the user is logged in
    login.ensureLoggedIn(),
